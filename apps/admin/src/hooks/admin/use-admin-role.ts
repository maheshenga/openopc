...
export async function fetchAdminRole(
  hasUser: boolean,
  api: Pick<typeof backendApi, 'get'> = backendApi,
): Promise<AdminRoleResponse> {
  if (!hasUser) return { isAdmin: false, role: null };

  const response = await api.get<AdminSessionResponse>('/session', {
    showErrors: false,
  });
  if (response.error) return { isAdmin: false, role: null };

  const session = response.data;
  if (!session || typeof session.userId !== 'string' || !Array.isArray(session.permissions)) {
...
export const useAdminRole = (options?: Partial<UseQueryOptions<AdminRoleResponse>>) => {
  ...
  return useQuery<AdminRoleResponse>({
    ...
    queryFn: () => fetchAdminRole(Boolean(user)),
