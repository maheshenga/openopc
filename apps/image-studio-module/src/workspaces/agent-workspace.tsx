...
export function AgentWorkspace({
  ...
  const maxReferences = selectedImageModel?.capabilities.max_reference_images ?? 0;
  const supportsReferences = selectedImageModel?.capabilities.reference_images === true && maxReferences > 0;
  ...
  const generateFromAnswer = async () => {
    ...
    try {
      const generated = await generateImage({
        ...
        onStatus: setGenerationJob,
        signal: controller.signal,
      });
      setResults(generated);
      await onAssetsChanged();
    } catch (reason) {
      if (!isAbortError(reason)) setError(openOpcErrorMessage(reason, 'Agent 生图失败'));
    } finally {
  ...
  const cancelGeneration = async () => {
    ...
    try {
      if (generationJob?.cancellable) {
        setGenerationJob(await cancelImageJob(generationJob.job_id));
      }
    } catch (reason) {
