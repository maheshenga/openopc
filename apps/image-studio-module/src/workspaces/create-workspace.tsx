...
export function CreateWorkspace({
  ...
  const maxReferences = selectedModel?.capabilities.max_reference_images ?? 8;
  const supportsReferences = selectedModel?.capabilities.reference_images === true && maxReferences > 0;
  const supportsNegativePrompt = selectedModel?.capabilities.supports_negative_prompt !== false;
  const supportsSeed = selectedModel?.capabilities.supports_seed === true;
  const availableRatios = selectedModel?.capabilities.aspect_ratios ?? RATIOS;
  const availableQualities = selectedModel?.capabilities.qualities ?? ['standard', 'high'];
  const maxOutputCount = Math.min(8, selectedModel?.capabilities.max_output_count ?? 8);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!isAbortError(reason)) setError(openOpcErrorMessage(reason, '生成失败'));
    } finally {
