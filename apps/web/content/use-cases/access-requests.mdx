...
policy that governs it. This writes up how we run that on OpenOPC — the
...
On OpenOPC, an access request in Slack triggers an agent. Each request runs in its
...
<Figure
  caption="The agent checking a request against policy and preparing a least-privilege grant"
  aspect="16/9"
/>
...
- **Isolation.** Every request runs in its own microVM sandbox on its own branch.
  The session can reach only the systems it's scoped to, and only what it's
...
<Figure
  caption="A prepared grant held for human approval, with its policy check and scope attached"
  aspect="16/9"
/>
