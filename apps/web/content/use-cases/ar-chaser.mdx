...
description: An accounts-receivable agent we run on OpenOPC — connected to Stripe, email, and the accounting ledger. It finds overdue invoices, sends the right reminder, logs every touch, and reconciles when payment lands.
date: "2026-02-11"
author: team
tags:
  - Finance
...
We run an accounts-receivable agent on OpenOPC that does this chasing on a daily
...
On OpenOPC, a daily cron triggers an agent. Each morning it spawns an isolated
...
<Figure
  caption="The agent's daily receivables run, grouped by how late each balance is"
  aspect="16/9"
/>
...
and injected at runtime, never shown to the model or written to logs.
...
- **Isolation.** Every run happens in its own per-task microVM sandbox. The
  session reads Stripe and the ledger and can send only the reminders and status
  notes it's scoped to; nothing else leaves the sandbox.
...
<Figure
  caption="A held reminder waiting on approval, with the balance and dispute flag attached"
  aspect="16/9"
/>
