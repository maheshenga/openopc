...
We run a churn-risk agent on OpenOPC that reads those signals every day and posts a
...
On OpenOPC, a daily cron triggers an agent. It spawns an isolated session (a cloud
...
<Figure
  caption="The daily at-risk list posted to the customer-success channel"
  aspect="16/9"
/>
...
- **Isolation.** Every run happens in its own microVM sandbox. The session can
  reach only the systems it's scoped to, and only the Slack post leaves the
  sandbox.
...
<Figure
  caption="An account's entry with its contributing signals and suggested next step"
  aspect="16/9"
/>
