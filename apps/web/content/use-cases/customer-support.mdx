...
description: The support agent we run on OpenOPC — connected to Plain, our codebase, and Stripe. It triages and resolves inbound threads, and stops for human approval on anything sensitive.
date: "2026-07-03"
author: team
tags:
  - Customer Support
...
We run our own customer support on OpenOPC with an AI agent connected to Plain
...
<KeyFacts>
  <Fact label="Team">OpenOPC — the company behind this platform</Fact>
...
Our support inbox in **Plain** is connected to an agent running on OpenOPC. Every
...
<Figure
  src="/usecases/support/support_1.JPEG"
  alt="The support agent replying inside a Plain thread"
/>
...
security question as much as a product one. The controls on OpenOPC:

- **Isolation.** Each thread runs in its own microVM sandbox on its own branch. A
  session can install, run, and experiment to reproduce a bug, and only what it's
  explicitly allowed to send leaves the sandbox.
...
<Figure
  ...
  src="/usecases/support/support_2.JPEG"
/>
