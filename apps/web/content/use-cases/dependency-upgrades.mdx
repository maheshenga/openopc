...
description: The upgrade agent we run on OpenOPC — a weekly cron that opens dependency PRs, runs the full suite in a sandbox, and only opens the PR when it's green.
date: "2026-02-11"
author: team
tags:
  - Engineering
...
We run an upgrade agent on OpenOPC that does the checking before it asks for a
...
On OpenOPC, a weekly cron triggers an upgrade agent. Each run spawns an isolated
