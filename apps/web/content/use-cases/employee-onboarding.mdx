...
our HR system. This writes up how we run that on OpenOPC — the connections, the
...
On OpenOPC, a new-hire record triggers an agent. Each new hire runs in its own
...
<Figure
  caption="The agent preparing a new hire's accounts and holding provisioning for approval"
  aspect="16/9"
/>
...
in the secrets manager and injected at runtime, never shown to the model or
written to logs.
...
question. The relevant controls on OpenOPC:

- **Isolation.** Every new hire runs in its own microVM sandbox on its own
  branch. The session can reach only the systems it's scoped to, and only what
  it's explicitly allowed to send leaves the sandbox.
- **Scoped secrets.** Each credential is encrypted in the secrets manager,
  injected into the sandbox at runtime, and never exposed to the model or the
  logs.
...
<Figure
  caption="A new hire's accounts and memberships held for human approval before provisioning"
  aspect="16/9"
/>
