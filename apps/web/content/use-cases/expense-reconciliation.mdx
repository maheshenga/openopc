...
description: The finance agent we run on OpenOPC — connected to Stripe, the bank feed, and Google Sheets. Each month it matches transactions against invoices, flags mismatches, and posts a summary, escalating anything it can't match.
date: "2026-04-15"
author: team
tags:
  - Finance
...
how we run that on OpenOPC — the connections, the steps, and the guardrails.
...
On OpenOPC, a monthly **cron** triggers a reconciliation agent. Each run spawns
...
<Figure
  caption="The finance agent matching transactions and flagging mismatches"
  aspect="16/9"
/>
...
relevant controls on OpenOPC:

- **Isolation.** Each run happens in its own microVM sandbox. The session can
  read the accounts it needs, and only the summary it writes to the sheet leaves
  the sandbox.
...
<Figure
  caption="An unmatched transaction escalated to a person to resolve"
  aspect="16/9"
/>
