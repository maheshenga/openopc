...
description: The inbox agent we run on OpenOPC — connected to Gmail, a help doc, and Linear. It labels every inbound email, drafts a reply, or files a task, and stops for approval before anything goes to a customer.
date: "2026-03-04"
author: team
tags:
  - Support
...
Our shared inbox in **Gmail** is connected to an agent running on OpenOPC. Every
...
<Figure
  caption="The inbox agent labelling a thread and drafting a reply"
  aspect="16/9"
/>
...
at runtime, never shown to the model or written to logs.
...
convenience one. The relevant controls on OpenOPC:

- **Isolation.** Each email runs in its own microVM sandbox. The session can read
...
<Figure
  caption="A drafted reply held at a human approval gate before it sends"
  aspect="16/9"
/>
