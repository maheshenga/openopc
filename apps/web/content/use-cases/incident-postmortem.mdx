...
We run an agent on OpenOPC that drafts the first version. When an incident
...
On OpenOPC, resolving an incident triggers an agent. It reads the incident channel
...
<Figure
  caption="The drafted postmortem opened as a doc PR for review"
  aspect="16/9"
/>
...
- **Isolation.** Every incident runs in its own microVM sandbox. The session
  reads the timeline, correlates the deploys and logs, and only the drafted doc
  PR leaves the sandbox.
...
<Figure
  caption="A correlated timeline lining up channel messages against deploys and log spikes"
  aspect="16/9"
/>
