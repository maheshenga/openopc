...
description: A monthly agent we run on OpenOPC — connected to Postgres, Stripe, and last month's update. It pulls the core metrics, compares them to last month and to plan, and drafts the update in our format for a founder to finalize.
date: "2026-03-04"
author: team
tags:
  - Finance
...
We run an agent on OpenOPC that does the gathering and the first draft. This is how
...
On OpenOPC, a monthly cron triggers an agent. It spawns an isolated session (a
...
<Figure
  caption="The drafted update with each metric set against last month and plan"
  aspect="16/9"
/>
...
- **Isolation.** Every run happens in its own per-task microVM sandbox. The
  session reads only the systems it's scoped to, and only the draft document leaves
  the sandbox.
...
<Figure
  caption="A read-only run with the source metrics traced back to Postgres and Stripe"
  aspect="16/9"
/>
