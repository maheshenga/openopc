...
We handle it with an agent on OpenOPC. People request snacks through a Slack
...
<Figure
  caption="A batched snack order posted for approval in Slack"
  aspect="16/9"
/>
...
- **Isolation.** Each weekly run executes in its own microVM sandbox, and only the
  prepared order and Slack post it's explicitly allowed to send leave the sandbox.
...
<Figure
  caption="The order held at the approval gate before checkout"
  aspect="16/9"
/>
