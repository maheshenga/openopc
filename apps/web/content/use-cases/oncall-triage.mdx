...
description: The triage agent we run on OpenOPC — connected to Sentry, our logs, and GitHub. It works up a first-pass diagnosis on every alert and only pages a human when it can't resolve it.
date: "2026-02-25"
author: team
tags:
  - SRE
...
We run a triage agent on OpenOPC that does the first pass. When an alert fires, the
...
On OpenOPC, each alert triggers an agent. When Sentry fires, the alert spawns an
...
<Figure
  caption="The triage agent posting a first-pass diagnosis in the incident channel"
  aspect="16/9"
/>
...
- **Isolation.** Every alert runs in its own microVM sandbox. The session can pull
  traces, logs, and deploy history to build the diagnosis; only the posted message
  and any page leave the sandbox.
...
<Figure
  caption="A high-severity alert paged to on-call with the diagnosis attached"
  aspect="16/9"
/>
