...
description: The QA agent we run on OpenOPC — connected to GitHub and our test environment. It checks out each PR, runs the suite, exercises the change, and posts the result.
date: "2026-07-05"
author: team
tags:
  - QA
...
We run a QA agent on OpenOPC that does this checking when the PR opens. This is how
...
On OpenOPC, each PR triggers an agent. On open and on every push, the PR spawns an
...
<Figure
  caption="The QA agent posting its result as a PR check"
  aspect="16/9"
/>
...
- **Isolation.** Every PR runs in its own microVM sandbox on its own branch. The
  ...
  only the reported result leaves the sandbox.
...
<Figure
  caption="A red QA result with the failing command and repro steps attached"
  aspect="16/9"
/>
