...
description: The release-notes agent we run on OpenOPC — connected to GitHub. On each release it reads the merged PRs since the last one, groups them, writes the notes, and opens a changelog PR.
date: "2026-05-06"
author: team
tags:
  - Engineering
...
We run a release-notes agent on OpenOPC that does it from the PR history. On each
...
On OpenOPC, each release triggers an agent. When a tag is pushed, the release
...
<Figure
  caption="The release-notes agent opening a changelog PR grouped by area"
  aspect="16/9"
/>
...
encrypted in the secrets manager and injected at runtime, never shown to the model
or written to logs.
...
- **Isolation.** Every release runs in its own microVM sandbox. The session reads
  the PR history and drafts the notes; only the changelog branch leaves the
  sandbox.
- **Scoped secrets.** The GitHub credentials are encrypted in the secrets manager
  and injected into the sandbox at runtime, never exposed to the model or the logs.
...
<Figure
  caption="A changelog PR with notes grouped by area, ready for review"
  aspect="16/9"
/>
