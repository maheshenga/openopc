...
description: An agent we run on OpenOPC — connected to the inbound questionnaire and our knowledge base of vetted answers and policies. It parses each question, drafts responses in the vendor's format, and flags anything it can't answer confidently.
date: "2026-04-15"
author: team
tags:
  - Sales
...
We run an agent on OpenOPC that drafts the answers from our vetted knowledge base.
...
On OpenOPC, each inbound questionnaire triggers an agent. It spawns an isolated
...
<Figure
  caption="The agent filling a CAIQ, with low-confidence rows flagged for review"
  aspect="16/9"
/>
...
Credentials are encrypted in the Secrets Manager and injected at runtime, never
...
- **Isolation.** Every questionnaire runs in its own per-task microVM sandbox. The
  session reads the inbound document and the knowledge base it's scoped to, and
  only the filled draft leaves the sandbox.
- **Scoped secrets.** The credentials for the questionnaire source and the
  ...
  at runtime, never exposed to the model or the logs.
...
<Figure
  caption="A returned draft with vetted answers filled and flagged rows left for security"
  aspect="16/9"
/>
