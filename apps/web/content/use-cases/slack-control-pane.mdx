...
OpenOPC, and the same setup works for any team.
...
<Figure
  caption="Asking the agent to invite an engineer to Linear"
  src="/usecases/slack/operation_1.JPEG"
/>
...
To give the agent a new capability, you **connect the platform to OpenOPC once**.
...
<Callout title="The setup" tone="accent">
  Connect Slack as the **channel**, connect each platform to OpenOPC as a scoped
...
- **Isolation.** Every request runs in its own microVM sandbox on its own branch.
  A session can reach only the platforms it's scoped to, and only what it's
...
<Figure
  src="/usecases/slack/operation_2.JPEG"
/>
