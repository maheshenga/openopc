...
We run a daily agent on OpenOPC that does the gathering. It reads each person's
...
On OpenOPC, a cron fires once each weekday morning and spawns an agent. It reads
...
<Figure
  caption="The daily standup summary posted to the team Slack channel"
  aspect="16/9"
/>
...
- **Isolation.** Every run happens in its own microVM sandbox. The session reads
  the day's activity, builds the summary, and only the Slack post leaves the
  sandbox.
...
<Figure
  caption="A per-person summary with a light prompt for a teammate who has nothing recorded"
  aspect="16/9"
/>
