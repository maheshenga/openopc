...
We run a feedback agent on OpenOPC that gathers those sources on a schedule,
...
On OpenOPC, a scheduled cron triggers an agent. It spawns an isolated session (a
...
<Figure
  caption="A Linear issue for a theme with representative quotes and a mention count"
  aspect="16/9"
/>
...
- **Isolation.** Every run happens in its own microVM sandbox. The session can
  reach only the sources it's scoped to, and only the Linear writes leave the
  sandbox.
...
<Figure
  caption="Themes ranked by mention count, each linked to its Linear issue"
  aspect="16/9"
/>
