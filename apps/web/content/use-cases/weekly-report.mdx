...
description: The reporting agent we run on OpenOPC — connected to our Postgres database and Slack. Every Monday it queries the metrics, writes the report with commentary on what moved, and posts it.
date: "2026-05-20"
author: team
tags:
  - Data
...
We run a reporting agent on OpenOPC that builds it. A Monday cron queries the
...
On OpenOPC, a Monday cron triggers a reporting agent. Each run spawns an isolated
...
<Figure
  caption="The reporting agent posting the weekly metrics report to Slack"
  aspect="16/9"
/>
...
- **Isolation.** Every run happens in its own microVM sandbox. The session queries
  the metrics and drafts the report; only the Slack message leaves the sandbox.
...
<Figure
  caption="The weekly report in Slack with commentary on what moved"
  aspect="16/9"
/>
