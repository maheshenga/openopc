...
const MAX_IMAGE_URL_LENGTH = 14_000_000;
...
function unique<T extends z.ZodTypeAny>(schema: T, maximum: number) {
  return z
    .array(schema)
    .min(1)
    .max(maximum)
...
function dataImage(value: string): { mimeType: OpenOpcImageMimeType; sizeBytes: number } | null {
  ...
  const mimeType = match?.[1] as OpenOpcImageMimeType | undefined;
  if (!mimeType || !OPENOPC_IMAGE_MIME_TYPES.includes(mimeType)) return null;
...
export const OpenOpcChatTextContentPartSchema = z
  .object({ type: z.literal('text'), text: z.string().min(1).max(1_000_000) })
  .strict();
...
export const OpenOpcChatImageUrlContentPartSchema = z
  ...
  .superRefine((part, context) => {
    ...
    if (!embedded && !validRemoteUrl(part.image_url.url)) {
...
const ChatPartsSchema = z
  ...
  .superRefine((parts, context) => {
    ...
    const bytes = images.reduce((total, part) => {
      const embedded = dataImage(part.image_url.url);
      return total + (embedded?.sizeBytes ?? part.image_url.size_bytes ?? 0);
    }, 0);
    if (bytes > OPENOPC_CHAT_MAX_TOTAL_IMAGE_BYTES) {
...
export const OpenOpcChatMessageSchema = z
  .object({
    ...
    content: z.union([z.string().max(1_000_000), ChatPartsSchema, z.null()]),
...
export const OpenOpcModelImageInputCapabilitySchema = z
  .object({
    ...
    accepted_mime_types: unique(OpenOpcImageMimeTypeSchema, OPENOPC_IMAGE_MIME_TYPES.length),
    purposes: unique(OpenOpcModelImagePurposeSchema, 2),
  })
  .strict();
...
export const OpenOpcModelCapabilitiesSchema = z
  .object({
    modalities: unique(z.enum(['text', 'image']), 2),
    vision: OpenOpcModelImageInputCapabilitySchema.optional(),
    attachment: OpenOpcModelImageInputCapabilitySchema.optional(),
  })
  .strict()
  .superRefine((value, context) => {
    if (Boolean(value.vision ?? value.attachment) !== value.modalities.includes('image')) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: 'image capability is incomplete' });
    }
    if (value.vision && !value.vision.purposes.includes('vision')) {
      context.addIssue({ code: z.ZodIssueCode.custom, message: 'vision purpose is required' });
    }
    if (value.attachment && !value.attachment.purposes.includes('attachment')) {
  ...
  .object({
    ...
    output: z
      .object({
        ...
        max_bytes_per_image: z.number().int().positive().max(OPENOPC_IMAGE_ASSET_MAX_BYTES),
        accepted_mime_types: unique(OpenOpcImageMimeTypeSchema, OPENOPC_IMAGE_MIME_TYPES.length),
        aspect_ratios: unique(OpenOpcImageAspectRatioSchema, 5),
        qualities: unique(OpenOpcImageQualitySchema, 2),
      })
      .strict()
...
export const OpenOpcImageEstimateCreateInputSchema = z
  .object({ model: z.string().min(1).max(512), input: OpenOpcImageGenerateInputSchema })
  .strict();
...
export const OpenOpcImageEstimateSchema = z
  .object({
    ...
    line_items: z
      .array(
        z
          .object({ label: z.string().min(1).max(255), credits: z.number().finite().nonnegative() })
          .strict(),
      )
      .max(32),
  })
...
export const OpenOpcImageAssetSchema = z
  .object({
    ...
    source: z
      .object({ job_id: z.string().uuid().nullable(), prompt: z.string().max(8000).nullable() })
      .strict(),
