...
Launch an installed sandboxed-web module through its project-bound handle:

```ts
const launch = await kortix.project(projectId).modules.launch(installationId);

iframe.src = launch.url;
```

Use the server-issued `launch.url` verbatim. The launch descriptor never returns
