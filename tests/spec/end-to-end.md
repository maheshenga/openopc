...
`STUDIO-13` Local Web Image Studio acceptance runs against a webpack Next dev server with `$env:E2E_BASE_URL='http://localhost:3300'; pnpm.cmd --filter Kortix-Computer-Frontend test:e2e:image-studio`. The browser opens the debug-only `/debug/image-studio` harness, which mounts the real `StudioShell`, Image Studio, and Assets views with a fixed project and `debug-image-studio-token`. The fake provider intercepts project-scoped capability and Agent Card discovery, signed estimate, one `503` task response followed by an idempotent replay, paginated task events, signed reference upload/finalize, cancellation, asset list/preview/download, and insufficient-credit/permission responses. The flow asserts the Authorization header on every project request, signed-upload headers and bytes, estimate approval propagation, durable `?task=` URL and reload recovery, reference reuse, source/reuse asset links, desktop/mobile layout bounds, nonblank screenshots, no horizontal overflow, no cancelled multimedia navigation, and no token/estimate/signed-URL leakage in browser logs. Screenshots are local test artifacts and are not committed.
...
`INIT-1` `kortix init -y` empty dir → `kortix.yaml` + `.kortix/` (Dockerfile, `.kortix/opencode/…`, canonical skill) written, agent skill wired (codex default → `AGENTS.md`; `--primary opencode|claude|cursor` → respective skill file/symlink), `git init -b main`. **Zero API calls.** Exit 0.
`INIT-2` `kortix init` when `kortix.yaml` exists, no `--force` → exit 1 (refuses).
...
`IAM-31` `PUT /projects/:id/agents/:agentName/scope {env?,connectors?}` (`manage`) — writes the `[[agents]].env`/`.connectors` allowlists into `kortix.yaml` (or legacy `kortix.toml`); empty body (`nothing_to_update`) → 400; malformed grant set → 400; unknown agent name → 404 (`agent_not_found`); caller with no project grant → 404.
